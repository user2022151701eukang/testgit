...
impl<'a> System<'a> for RenderingSystem<'a> {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        self.draw_text(&format!("Moves: {}", gameplay.moves_count), 525.0, 100.0);
        let fps = format!("FPS: {:.0}", timer::fps(self.context));
        self.draw_text(&fps, 525.0, 120.0);
        
        // 添加通关提示
        if let GameplayState::Won = gameplay.state {
            self.draw_text("You Win!", 525.0, 160.0);
            self.draw_text("Press SPACE to restart", 525.0, 180.0);
        }
        // 添加失败提示
        else if let GameplayState::Failed = gameplay.state {
            self.draw_text("You Failed!", 525.0, 160.0);
            self.draw_text("Press SPACE to restart", 525.0, 180.0);
        }
